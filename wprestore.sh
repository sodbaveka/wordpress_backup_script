...
scp $user_login@$ftp_server:"$source_folder_path"/.temp_file.tmp .temp_file.tmp && echo "list of files to download copied"

# Downloading files from ftp server with secure connection
while read ligne
do 		
	sftp $user_login@$ftp_server:"$source_folder_path"/"$ligne"
